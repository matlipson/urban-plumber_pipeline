...
obs_contact = 'Simone Kotthaus (kotthaus@ipsl.polytechnique.fr); Sue Grimmond (c.s.grimmond@reading.ac.uk)'
obs_reference = 'Bjorkegren et al. (2015): https://doi.org/10.1016; Kotthaus and Grimmond (2013a): https://doi.org/10.1016/j.uclim.2013.10.002; Kotthaus and Grimmond (2013b): https://doi.org/10.1016/j.uclim.2013.10.001/'
obs_comment = 'Observations from KSSW tower'
photo_source='S. Grimmond'
...
def main():
    ...
    if create_rain_file:
        ...
        rain_sites = ['UKE00105915',  # HAMPSTEAD, UK       51.5608     0.1789
                      'UKM00003772']  # HEATHROW, UK        51.4780    -0.4610
...
def import_obs(sitedata,siteattrs):
    '''During co-author comments on manuscript new data came to light which supersedes input data provided to modellers
    This data is in the original 30 min processing periods, other data had been reprocessed to 60 min
    In addition, the InputData file inludes gap-filled variables, the location of which are parsed through gaps in AnalysisData
    see email from Helen Ward 23 August 2021 at 5:55 pm
    '''

    # read data csv
    print('reading raw data file')

    raw = pd.read_csv(f'{datapath}/{sitename}/AnalysisData_Kc.txt', na_values=-999)
    raw2 = pd.read_csv(f'{datapath}/{sitename}/InputData_Kc.txt', na_values=-999)

    # get times from data and reindex
    # times = pd.date_range(start='2011-01-01 00:30:00', end='2013-05-01 0:00:00', freq='30Min')
    times = pd.date_range(start='2012-01-01 00:30', end='2014-01-01 00:00', freq='30Min')
    raw.index = times
    raw2.index = times

    # use KSSW only
    raw = raw['2012-04-04 00:00:00':]
    raw2 = raw2['2012-04-04 00:00:00':]
    ...
    # create dataframe without gap filling, used to find where gaps have been filled
