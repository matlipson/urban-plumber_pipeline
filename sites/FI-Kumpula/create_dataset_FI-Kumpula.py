...
obs_reference = 'Karsisto, Fortelius, Demuzere, Grimmond, Oleson, Kouznetsov, Masson and Järvi (2016): https://doi.org/10.1002/qj.2659'
obs_comment = 'Inhomogeneous footprint, land surface characteristics and turbulent flux data restricted to sectors 180-320 (Ku3 in Karsisto et al., 2016)'
...
def import_obs(sitedata,siteattrs):
    ...
    raw1 = pd.read_csv(f'{datapath}/{sitename}/Helsinki_Kumpula_Forcing_mjl.txt', na_values='NaN', delim_whitespace=True)
    raw2 = pd.read_csv(f'{datapath}/{sitename}/Helsinki_Kumpula_Eval.txt', na_values='NaN', delim_whitespace=True)
    ...
    wind_dir = pd.Series(pipeline_functions.convert_uv_to_wdir(raw1['Wind_E'],raw1['Wind_N']),index=times)
    # remove wind sectors with flow disturbance from analysis: "urban" (Ku1) and "road" (Ku2)
    df['Qle'] = df['Qle'].where(~wind_dir.between(0,180))
    df['Qle'] = df['Qle'].where(~wind_dir.between(320,360))
    df['Qh'] = df['Qh'].where(~wind_dir.between(0,180))
    df['Qh'] = df['Qh'].where(~wind_dir.between(320,360))
