...
def import_obs(sitedata,siteattrs):
    ...
    # remove golf course wind sectors
    df[analysis_fluxes] = df[analysis_fluxes].where(~wind_dir.between(75,285))

    # reweight surface cover fractions to residential sector
    wd = raw.where( ~wind_dir.between(75,285) ) 
