...
def import_obs(sitedata,siteattrs):
    ...
    raw1 = pd.read_csv(f'{datapath}/{sitename}/alpha04.dat', na_values='nan', delim_whitespace=True, skiprows=[0,1,2,3,4,6])
    raw2 = pd.read_csv(f'{datapath}/{sitename}/AU-Preston_flux.csv', na_values='nan', usecols=range(4,11))
    raw3 = pd.read_csv(f'{datapath}/{sitename}/PRESTON_CITIES_13092011.csv')

    # get times from data and reindex
    times = pd.date_range(start='2003-08-12 13:30:00', end='2004-11-28 23:00:00', freq='30Min')
    raw1.index = times
    raw2.index = times
    raw3.index = times

    # remove previously gap filled data
    df1 = pd.DataFrame(index=times)
    df1 = df1.assign(
            SWdown = raw1['SWdown'].where(raw1['SWdown.1']==1),
            LWdown = raw1['LWdown'].where(raw1['LWdown.1']==1),
            Tair   = raw1['Tair'].where(raw1['Tair.1']==1),
            Qair   = raw1['Qair'].where(raw1['Qair.1']==1),
            PSurf  = raw1['Psurf'].where(raw1['Psurf.1']==1),
            Rainf  = raw1['Rainf'].where(raw1['Rainf.1']==1),
            Snowf  = np.nan,
            Wind_N = raw1['Wind_N'].where(raw1['Wind_N.1']==1),
            Wind_E = raw1['Wind_E'].where(raw1['Wind_E.1']==1)
            )
    ...
    df = df.assign(
            ...
            Qh     = raw2['QHup'],
            ######################
            # Qg     = raw3['QG'], # not actual ground heat flux
            Qtau   = raw3['tau'],
            )
