...
def main():
    ...
    if create_rain_file:
        ...
        rain_sites = ['GR000016754',   # HERAKLION, GR         35.3331    25.1831
                      'GR000167460',   # SOUDA AIRPORT, GR     35.4831    24.1167
                      'era_daily_rain'] # could not find other records for 6 remaining days, fill with era (i.e. no change to bias on those days)
...
def import_obs(sitedata,siteattrs):
    ...
    raw1 = pd.read_excel(f'{datapath}/{sitename}/HECKOR_EddyPro_CNR_July_19-20.xls', na_values=-9999, sheet_name='Sheet1', skiprows=[1])
    raw2 = pd.read_csv(f'{datapath}/{sitename}/UF_Her_13_20190429_20200731_20200931_01.csv', 
             skiprows=[0], parse_dates=[[1,2]], index_col=[0])

    # get times from data and reindex
    times = pd.date_range(start='2019-07-01 00:00', end='2020-07-01 00:00', freq='30Min')
    raw1.index = times

    # resample rain
    rain = raw2['Rain Gauge [mm]'].resample('30Min').sum()
    rain.index.name=None

    # create dataframe in ALMA format
    df = pd.DataFrame(index=times)
    df = df.assign(
            SWdown = raw1['SWdown_Avg'],
            LWdown = raw1['LWdown_Avg'],
            Tair   = raw1['air_temperature'],
            Qair   = raw1['specific_humidity'],
            PSurf  = raw1['air_pressure'],
            Rainf  = rain/1800.,
            Snowf  = np.nan,
            Wind_N = pipeline_functions.convert_wdir_to_uv(
                        speed=raw1['wind_speed'],
                        wind_dir_from=raw1['wind_dir'],
                        )[1],
